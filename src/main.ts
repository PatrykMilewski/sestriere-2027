...
function shell(): string {
  ...
  <div class="controls"><div><span class="control-label">Jedzie nas</span><div class="segmented" role="group" aria-label="Liczba osób"><button data-party="2" aria-pressed="true">2 osoby</button><button data-party="4" aria-pressed="false">4 osoby</button></div></div><div><span class="control-label">Termin · 2027</span><div class="segmented" role="group" aria-label="Termin wyjazdu"><button data-week="early" aria-pressed="true">16–23 sty</button><button data-week="late" aria-pressed="false">23–30 sty</button><button data-week="feb" aria-pressed="false">6–13 lut</button><button data-week="march" aria-pressed="false">6–13 mar</button></div></div><div class="exchange"><b>1 € = 4,30 zł</b><span>Kurs do planowania, nie kurs banku</span></div></div><div id="budget" aria-live="polite"></div><div id="comparison"></div></section>
  ...
  ${transportSection()}
  <section class="section" id="noclegi">${heading('02 / BAZA W GÓRACH', 'Klucze do dobrego tygodnia.', 'Cena na osobę za 7 nocy, przy podanej liczbie osób w lokalu. Sprawdź układ łóżek: sofa w Casa Rossa, łóżko piętrowe w Nube.')}<div id="stays"></div><div id="group-stays"></div><p class="footnote">Styczeń: odczyty 19–20.09.2026. Luty i marzec: Nube dla 4 osób oraz loty sprawdzone 22.09.2026; dostępność może się zmienić. Odległości według opisów ofert, nie pomiaru zimowej drogi pieszej. Nic nie zostało zarezerwowane.</p></section>
  ${skiSection()}${itinerarySection()}${sourceSection()}</main>
  <footer><a class="brand" href="#">▲ ALPY / 27</a><p>Plan dla ekipy · Sestriere, styczeń–marzec 2027<br>Styczeń: odczyty 19–20.09.2026. Nowe terminy: odczyty 22.09.2026.</p><a href="#plan">Wróć do kosztów ↑</a></footer>`;
}
...
function budgetMarkup(): string {
  const t = totals();
  const rows = [
    ['Loty — snowboard i walizka wliczone', t.flight === null ? 'Do potwierdzenia' : pln(t.flight / 2), `Na osobę w obie strony · ½ walizki 20 kg (jedna na 2 osoby) + własny snowboard. Luty i marzec: bagaże szacunkowo, do potwierdzenia${party === 4 ? ' · ekstrapolacja ceny dla pary' : ''}`],
  ...
  return `<div class="budget-layout"><div class="budget-table">${rows.map(([title, price, desc]) => `<div class="cost-row"><div><h3>${title}</h3><p>${desc}</p></div><strong>${price}</strong></div>`).join('')}<div class="budget-extra"><span>Jedzenie: dodatkowo ok. <b>${pln(t.food / party)}</b> / osoba</span><span>Ubezpieczenie sportowe, nośnik skipassu i dojazd do GDN: osobno</span></div></div><aside class="total-ticket"><div class="ticket-top"><span>WYBRANY WARIANT</span><b>${party} OS.</b></div><p>${weeks[week].label} 2027</p><span class="total-label">Loty + nocleg + skipass + transport</span><span class="included-badge">Snowboard ujęty w kalkulacji · nowe terminy: szacunek</span><strong class="total-number">${t.core === null ? '—' : money(t.core / party)}<span>${t.core === null ? '' : ' zł'}</span></strong><div class="ticket-per">${t.core === null ? 'Brakuje danych do zsumowania wariantu.' : 'na osobę za cały wyjazd'}</div><p class="ticket-note">${t.core === null ? 'Otwórz linki do ofert poniżej. Brak ceny nie oznacza braku miejsc.' : `Z jedzeniem: ok. ${pln((t.core + t.food) / party)} na osobę. Suma zawiera szacunkową rezerwę transportową.${selectedStay === 'nube' ? ' Dojazd Sestriere–Borgata dodatkowo: koszt niepotwierdzony.' : ''}`}</p><div class="ticket-bottom">7 NOCY <span>✳</span> 6 DNI JAZDY</div></aside></div>`;
}
function comparisonMarkup(): string {
  return `<div class="comparison"><div><p class="eyebrow">CZTERY TERMINY</p><h3>Porównaj koszt na osobę.</h3><p>Ten sam wybrany nocleg i liczba osób. Loty Wizz, własny snowboard, ½ walizki 20 kg, 7 noclegów, 6 dni skipassa i rezerwa transportowa. Dla lutego i marca bagaże są szacunkiem według wcześniejszych stawek; cena noclegu jest sprawdzona tylko dla 4 osób w Nube d’Argento. Brakujących cen nie liczymy jako zero.</p></div><div class="compare-weeks">${(['early', 'late', 'feb', 'march'] as Week[]).map(w => {const t = totals(w); return `<div class="compare-week ${w === week ? 'selected' : ''}"><span>${weeks[w].label}</span><strong>${t.core === null ? 'Niepełna wycena' : pln(t.core / party)}</strong><small>na osobę · grupa ${party} osób · ${t.stay.name}</small></div>`;}).join('')}</div></div>`;
}
function flightsMarkup(): string {
  const costs = week === 'early' ? [1136, 305.43, 1002.88, 2444.31] : week === 'late' ? [1036, 278.08, 1002.88, 2316.96] : [week === 'feb' ? 1664 : 742, 278.08, 1002.88, weeks[week].flight];
  const exact = (n: number) => n.toLocaleString('pl-PL', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' zł';
  return `<div class="flight-card"><div class="baggage-included"><strong>Snowboard ujęty w kalkulacji · luty i marzec: opłata szacunkowa</strong><p>Łączna cena na osobę obejmuje bilet, własny pokrowiec snowboardowy oraz połowę kosztu wspólnej walizki 20 kg — wszystko w obie strony. Dla lutego i marca: bilet ze Skyscannera z 22.09.2026, opłaty bagażowe z wcześniejszego sprawdzenia stycznia — wymagają potwierdzenia w koszyku. Rozpiska poniżej pokazuje składniki tej sumy; nie doliczaj ich ponownie.</p></div><div class="flight-route"><div><span>WYLOT · ${weeks[week].start.slice(8)}.${weeks[week].start.slice(5, 7)}</span><strong>GDN <i>→</i> TRN</strong><p>11:55 → 14:10</p></div><div class="flight-airline">WIZZ AIR<span>bezpośrednio · taryfa regularna</span></div><div><span>POWRÓT · ${weeks[week].end.slice(8)}.${weeks[week].end.slice(5, 7)}</span><strong>TRN <i>→</i> GDN</strong><p>15:00 → 17:10</p></div></div><div class="flight-breakdown"><span>Bilet na osobę w obie strony <b>${exact(costs[0] / 2)}</b></span><span>½ kosztu walizki 20 kg w obie strony <b>${exact(costs[1] / 2)}</b></span><span>Własny snowboard w obie strony <b>${exact(costs[2] / 2)}</b></span><span>Razem na osobę <b>${exact(costs[3] / 2)}</b></span></div><div class="flight-links">${external(flightLink(week, party), `Skyscanner · ${party} osoby`, 'button primary')}${external(wizzLink(week, party), 'Sprawdź koszyk Wizz Air', 'button outline')}<p>Mały plecak 40 × 30 × 20 cm na osobę w cenie. Bez wyboru miejsc i Priority. Stawki bagażowe z formularza Wizz; drugi snowboard obliczony z tej samej stawki 250,72 zł / lot. Dla 4 osób przyjęto 2 × cenę pary, nie osobną ofertę dla 4 pasażerów.</p></div></div>`;
}
...
function bindControls(map: L.Map): void {
  document.addEventListener('click', event => {
    ...
    if (button.dataset.week && ['early', 'late', 'feb', 'march'].includes(button.dataset.week)) {week = button.dataset.week as Week; refresh();}
